...
pimcore.registerNS("pimcore.plugin.processmanager.callbackSettingPanel");
pimcore.plugin.processmanager.callbackSettingPanel = Class.create({

    getPanel: function () {
        if (this.layout == null) {
            this.layout = new Ext.Panel({
                title: t("plugin_process_manager_callback_settings"),
                border: false,
    ...
    createGrid: function(response) {
        ...
        if(pimcore.globalmanager.get("user").isAllowed("plugin_process_manager_configure")) {
            ...
            var copy = {
                header: t('plugin_process_manager_copy'),
                xtype: 'actioncolumn',
                width: 70,
                items: [{
                    tooltip: t('plugin_process_manager_copy'),
                ...
                items: [
                    {
                        ...
                        handler: function (grid, rowIndex) {
                            ...
                            var modal = new Ext.Window({
                                ...
                                buttons: [
                                    {
                                        text: t("plugin_process_manager_delete_confirm_callback_setting"),
                                        ...
                                        handler: function () {
                                            ...
                                            Ext.Ajax.request({
                                                ...
                                                success: function (response) {
                                                    Ext.getCmp('plugin_process_manager_callback_settings_panel').store.reload();
                                                    modal.close();
                                                }.bind(this)
                                            });
                                        }
        ...
        var gridConfig = {
            ...
            store: this.store,
            id : 'plugin_process_manager_callback_settings_panel',
            columns: gridColumns,
            loadMask: true,
            columnLines: true,
